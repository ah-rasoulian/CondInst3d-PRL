...
def dice_coefficient(x: torch.Tensor, target: torch.Tensor, fg_weight: float = 1.0, eps: float = 1e-5):
    """
    x, target: [N, ...] where N = num_instances (or batch)
    x is probabilities in [0,1], target is {0,1}
    returns: [N] dice loss per instance
    """
    n_inst = x.size(0)
