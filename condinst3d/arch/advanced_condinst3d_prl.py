...
from condinst3d.evaluator.metrics import AveragePrecision, DetectionConfusionMatrix, GlobalConfluentInstanceRecall
...
class AdvancedCondInst3dPRL(pl.LightningModule):
    ...
    def compute_detections_and_segmentations(self, inputs: Tensor) -> List[Dict[str, List[Tensor]]]:
        """
        Same as before, but keep producing per-patch instance logits.
        Expected inputs: [B, P, Ch, ph, pw, pd]
        Returns per_image list length B, each key maps to list length P.
        """
        B, P = inputs.shape[:2]

        flat_inputs = inputs.reshape(B * P, *inputs.shape[-4:])  # [B*P, Ch, ph, pw, pd]
        outputs = self.forward(flat_inputs)

        detections, per_patch_instances = self._compute_detections(outputs)  # len = B*P
        ...
        img_idx = inst_list.get_image_indices()  # [M] in [0..B*P-1]
        for flat_i in range(len(detections)):
            sel = (img_idx == flat_i)
            detections[flat_i]["onehot_logits"] = inst_logits[sel]  # [Ki,1,ph,pw,pd]
        ...
        for flat_i, det in enumerate(detections):
            ...
            per_image[b]["offset"].append(det["offset"])
    ...
    def merge_prediction(
        ...
        out = [merge_prediction(per_img, output_shape=output_shape, group_thresh=group_thresh, stride_mode="mean") for per_img in det_and_seg]
        return out
